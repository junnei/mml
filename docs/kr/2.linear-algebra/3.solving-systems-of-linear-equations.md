...
연립 일차 방정식은 $Ax = b$ 로 나타낼 수 있다고 [Chapter 1](./2-1#요약) 에서 확인했고,
...
정확히 말하자면, 첨가 행렬(Augmented matrix)를 사다리꼴 형태(row-echelon form)로 바꾸어주는 알고리즘이다.
...
각 행에서 0이 아닌 첫번째 요소를 pivot 이라고 하는데, pivot 이 존재하는 열을 pivot column 이라고 한다.

위의 과정에서 $v)$ 에서 사다리꼴 형태의 행렬이 완성된 것을 확인할 수 있고, 이 행렬은 항상 계단 구도를 가진다.
