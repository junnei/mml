...
마찬가지로, discrete random variable $X\sim p(x)$의 함수 $g: \R \to \R$의 기댓값은 다음과 같다.
$$
\begin{align}
...
<p align="center">
<img src = "https://user-images.githubusercontent.com/59478946/183128314-7cc8a567-1961-4efc-bd48-e958a7e66dcc.png"/>
</p>
...
이때, 식(9)에서 표현한 variance는 새로운 확률 변수 $Z := (X - \mu)^2$ 의 평균값이라고 해석할 수 있다. 경험적으로 식(9)의 variance 를 추정할 때, 우리는 다음의 두 알고리즘에 의존해야 합니다. 1) 데이터를 사용하여 식(7)의 empirical mean을 계산하는 알고리즘, 2) 앞서 구한 추정값 $\hat{\mu}$을 사용하여 variance를 계산하는 알고리즘.
그러나 variance를 raw-score formula 라고 불리는 식(10)으로 재정리하면 두 알고리즘을 거치지 않아도 된다. 
...
식(10)은 "제곱의 평균에서 평균의 제곱을 뺀 값"이라고 표현할 수 있다. 이는 곧 $x_i$ 데이터를 쌓음과 동시에 $x_i^2$을 계산할 수 있다는 점에서 하나의 알고리즘에만 의존한다는 것을 의미한다. ($x_i는 i번째 관측값을 말한다)
그러나 variance의 raw-score formula 식(10)은 수적으로 불안정하다는 점이 있다. 이는 머신러닝에서 bias-variance decomposition를 도출할 때 유용하게 사용된다. <br> <br>
...
위 식은 raw-score formula 식(10)의 2배로 구성된 것을 확인할 수 있다. 이는 기하학적으로 보면, pairwise 거리와 점 집합의 중심으로부터의 거리 사이에 동등성이 있다는 것을 의미한다.
...
마지막 항목인 식(19)의 반대는 성립하지 않을 수 있다. 즉, 두 확률변수가 covariance = 0 의 값을 갖더라도 statistical independence 하지 않을 수 있다. 이는 covariance 가 오직 linear dependence 만을 측정한다는 사실을 알면 된다. 그러므로 nonlinearly dependent인 확률변수들의 covariance는 0의 값을 가질 수 있다.
...
식(120)은 $z$에 대한 지식이 주어졌을 때, 분포 $x$와 $y$가 분해된다 라고 이해할 수 있다. 이는 확률의 곱셈규칙을 이용하여 다음의 식을 얻을 수 있다.
$$
\begin{align}
...
식(20)와 식(21)의 우변을 비교해보면, 다음의 관계식을 얻을 수 있다. 
$$
\begin{align}
p(x|y,z) = p(x|z)p(y|z)
\end{align}
...
<p align="center">
    <img src="https://user-images.githubusercontent.com/59478946/183204856-dc9440ad-baa3-4bbc-a5ac-8381974de026.png"/>
</p>
